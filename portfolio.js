...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // SMOOTH + LAZY IFRAME LOADING (Vimeo embeds are heavy, so load them only when near viewport)

    const allIframes = document.querySelectorAll('iframe');

    // Fade in on load (works for both normal + lazy iframes)
    ...
    // Lazy-load iframes that use data-src (recommended for Vimeo)
    ...
    if ('IntersectionObserver' in window && lazyIframes.length) {
        ...
        // Fallback: just load them normally if IntersectionObserver isn't supported
