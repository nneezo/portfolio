...
(function() {
    ...
    // Initialize particles lazily (prevents slowing down first page load)
    const prefersReducedMotion = window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches;

    function lazyInitOnce() {
        if (prefersReducedMotion) return;
        if (window.cursorParticles) return; // already init
        initParticles();
    }

    // Wait until user interacts (mousemove/touchstart) then init once
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', () => {
            window.addEventListener('mousemove', lazyInitOnce, { passive: true, once: true });
            window.addEventListener('touchstart', lazyInitOnce, { passive: true, once: true });
        }, { once: true });
    } else {
        window.addEventListener('mousemove', lazyInitOnce, { passive: true, once: true });
        window.addEventListener('touchstart', lazyInitOnce, { passive: true, once: true });
